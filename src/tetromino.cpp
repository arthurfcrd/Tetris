...
bool isValidTile(Point point, Grid g){ // checks if a tile is within the grid and is not occupied
    return point.x >= 0 && point.x < g.width && point.y >= 0 && point.y < g.height && g.grid[point.x][point.y] == Color::NONE;
}

void Tetromino::move(Grid g, int dx, int dy){
...
void Tetromino::rotate(Grid g, int direction){ // direction = 1 for clockwise, -1 for counterclockwise
    Point newTiles[4];
    for (int i = 0; i < 4; i++){
        Point point = tiles[i];
